...
const parseData = () => {
  ...
  return rows.map((str) => {
    ...
    const [min, max] = tokens[0].split("-").map((t) => parseInt(t));
    const character = tokens[1].replace(":", "");
...
const checkPasswordValidity = ({ policy, password }) => {
  const chars = password.split("").filter((char) => char === policy.character);

  if (chars.length < policy.min || chars.length > policy.max) return false;
...
const main = () => {
  ...
  // console.log(passwordPolicyPairs);
  let count = 0;
